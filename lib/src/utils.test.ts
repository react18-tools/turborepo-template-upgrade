...
describe("utils", () => {
  ...
  describe("getBaseCommit", () => {
    ...
    test("should calculate base commit from git history when no lst file", ({ expect }) => {
      ...
      mockExecSync
        .mockReturnValueOnce(firstCommitDate)
        .mockReturnValueOnce(templateCommits);
    ...
    test("should handle empty template commits", ({ expect }) => {
      ...
      mockExecSync
        .mockReturnValueOnce("2023-01-01 10:00:00 +0000")
        .mockReturnValueOnce("");
  ...
  describe("resolvePackageJSONConflicts", () => {
    test("should call resolveConflicts with correct configuration", async ({ expect }) => {
      ...
      expect(resolveConflicts).toHaveBeenNthCalledWith(1, expect.objectContaining({
      ...
      expect(resolveConflicts).toHaveBeenNthCalledWith(2, expect.objectContaining({
        include: ["**/package.json"],
    ...
    test("should handle missing optional files in custom strategy", async ({ expect }) => {
      ...
      mockExistsSync.mockImplementation((path) => {
        ...
        return !pathStr.includes("rebrand.js") && 
      ...
      expect(resolveConflicts).toHaveBeenCalledWith(expect.objectContaining({
